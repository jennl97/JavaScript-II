...
let fullName = [];
 runners.forEach((type) =>{
    fullName.push(`${type.first_name}, ${type.last_name}`);
});
// fullName = runners.forEach((type) => {
//     runners.map((runner) => {
...
let allCaps = [];
allCaps = runners.map((type) => {
    return type.first_name.toUpperCase();
})
console.log(allCaps); 
...
let largeShirts = [];
largeShirts = runners.filter((type) => {
    return type.shirt_size === "L";
});
